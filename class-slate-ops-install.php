...
async function loadCS() {
  ...
  view(`
    <div class="min-h-[calc(100vh-120px)] bg-[#f6f7f8]">
      <div class="mx-auto max-w-7xl px-4 py-6">

        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <h2 class="m-0 text-xl font-semibold text-slate-900">Customer Service</h2>
              <div class="mt-1 text-sm text-slate-500">Complete intake, assign SO#s, and keep jobs moving.</div>
            </div>
            <div class="flex flex-wrap gap-3 justify-end">
              <div class="rounded-xl border border-slate-200 bg-white px-4 py-3">
                <div class="text-xs text-slate-500">Pending Intake</div>
                <div class="mt-1 text-2xl font-semibold text-slate-900">${portalNeeds.length}</div>
              </div>
              <div class="rounded-xl border border-slate-200 bg-white px-4 py-3">
                <div class="text-xs text-slate-500">Needs SO#</div>
                <div class="mt-1 text-2xl font-semibold text-slate-900">${manualNeeds.length}</div>
              </div>
              <div class="rounded-xl border border-slate-200 bg-white px-4 py-3">
                <div class="text-xs text-slate-500">Active Jobs</div>
                <div class="mt-1 text-2xl font-semibold text-slate-900">${activeJobs.length}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
            <input id="cs-search" class="input w-full rounded-lg border-slate-300 bg-white text-sm" type="text" placeholder="Search SO#, VIN, customer, dealer..." />
            <button type="button" class="btn secondary" id="cs-search-clear">Clear</button>
          </div>
          <div class="mt-2 text-xs text-slate-500" id="cs-search-count"></div>
        </div>
        ...
        <div class="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between">
              <div>
                <div class="text-sm font-semibold text-slate-900">Recent Intake</div>
                <div class="mt-0.5 text-xs text-slate-500">Portal jobs waiting on intake.</div>
              </div>
              ${portalNeeds.length ? `<span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">${portalNeeds.length}</span>` : ``}
            </div>
            <div class="mt-3">
              ${(portalNeeds.slice(0,6).map(j => `
                <div class="py-3 border-b border-slate-100">
                  <div class="text-sm font-semibold text-slate-900">${escapeHtml(j.customer_name||'—')}</div>
                  <div class="mt-0.5 text-xs text-slate-500">Dealer: ${escapeHtml(j.dealer_name||'—')} • VIN: <span class="mono">${escapeHtml((j.vin||j.vin_last8||'').slice(-6)||'—')}</span></div>
              ...
              `).join('')) || `<div class="text-xs text-slate-500 py-3">No portal jobs pending intake.</div>`}
            </div>
          </div>
          ...
          <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between">
              <div>
                <div class="text-sm font-semibold text-slate-900">Active Jobs</div>
                <div class="mt-0.5 text-xs text-slate-500">Quick visibility for updates.</div>
              </div>
              ${activeJobs.length ? `<span class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-700">${activeJobs.length}</span>` : ``}
            </div>
            <div class="mt-3">
              ${(activeJobs.slice(0,6).map(j => `
                <div class="py-3 border-b border-slate-100">
                  <div class="text-sm font-semibold text-slate-900">SO# <span class="mono">${escapeHtml(j.so_number||'—')}</span> • ${escapeHtml(j.customer_name||'—')}</div>
                  <div class="mt-0.5 text-xs text-slate-500">Status: ${fmtStatus(j.status)} • Tech: ${escapeHtml(j.assigned_name||'—')}</div>
                  <div class="mt-2">
                    <button class="btn secondary small-btn" data-open-job="${j.job_id}">View</button>
                  </div>
                </div>
              `).join('')) || `<div class="text-xs text-slate-500 py-3">No active jobs.</div>`}
            </div>
          </div>
        </div>
        ...
        <div class="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div class="collapse-body" id="collapse-manual">
            <div class="p-5">
        ...
        <div class="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div>
            <div class="text-sm font-semibold text-slate-900">Work Queue</div>
            <div class="mt-0.5 text-xs text-slate-500">Search and open jobs from one list.</div>
          </div>
          <div class="mt-3 overflow-auto rounded-xl border border-slate-200">
            <table class="table" data-cs-table="1" id="cs-directory">
              <thead>
                <tr>
                  <th>SO#</th>
                  <th>Customer</th>
        ...
        <div class="mt-4 rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div class="collapse-body" id="collapse-active" ${!activeJobs.length ? 'style="display:none;"' : ''}>
            <div class="p-5">
              <table class="table" data-cs-table="1">
                <thead><tr><th>SO#</th><th>Customer</th><th>VIN</th><th>Status</th><th>Assigned</th><th></th></tr></thead>
                <tbody>
                  ${activeJobs.map(j=>`
                    <tr>
                      <td class="mono">${escapeHtml(j.so_number||'—')}</td>
