...
class Slate_Ops_REST {
  ...
  public static function update_settings($req) {
    ...
    $dealers = array_values(array_unique($dealers));
    update_option('slate_ops_dealers', $dealers);

    $sales_people_payload = $body['sales_people'] ?? [];
    if (is_string($sales_people_payload)) {
      $sales_people_payload = preg_split('/\r\n|\r|\n/', $sales_people_payload);
    }
    $sales_people = [];
    if (is_array($sales_people_payload)) {
      foreach ($sales_people_payload as $sales_person) {
        $sales_person = trim(sanitize_text_field((string) $sales_person));
        if ($sales_person !== '') {
          $sales_people[] = $sales_person;
        }
      }
    }
    $sales_people = array_values(array_unique($sales_people));
    update_option('slate_ops_sales_people', $sales_people);
